...
public class login extends Application{
   ...
   private funcontrol control = new funcontrol();
    ...
    public void start(Stage stage) {
        ...
        VBox v = new VBox(10, new Label("Usuário"), txtUsuario,
                               new Label("Senha"), txtSenha, btn);
        ...
        for (Funcionario f : control.getLista()) {
            if (f.getLogin().equals(usuario) && f.getSenha().equals(senha)) {
                ...
                try {
                    switch (f.getFuncao()) {
                        case 1:
                            // Gerente → abre tela intermediária
                            MenuGerente menu = new MenuGerente(stage, f); // podemos passar o usuário logado se precisar
                            menu.start(new Stage());
                            stage.hide();
                            break;

                        case 2:
                            // Atendente → tela ainda não implementada
                            System.out.println("Atendente logado (tela ainda não implementada).");
                            break;

                        case 3:
                            // Caixa → tela ainda não implementada
                            System.out.println("Caixa logado (tela ainda não implementada).");
                            break;
